...
#' @export
#'
soi<-function(Y,X,Z,Q,all=TRUE,goldsmith=FALSE,mcmc.controls=list("nr.iterations"=1000,"burnin"=100),plot=TRUE)
{
nVox=dim(X)[1]
nPat = dim(Z)[1]
...
burnin = mcmc.controls$burnin
niter = mcmc.controls$nr.iterations
...
for (iter in 1:niter)
{
  ...
  m <- (y - beta%*%x)%*%z
  m <- t(m)/sigma2
  alpha <- t(rmvnormcanon(1,m,zz/sigma2))
  ...
  if(all)
  {
    ...
    if (do.gamma)
      ...
      Q0 <- Q0[w,w]
      m<-m[w]
      beta[w] <- t(rmvnormcanon(1,m,Q0))
      beta[-w] <- 0
    }
    else
    {
      beta <- t(rmvnormcanon(1,m,Q0))
    }
  }
  ...
  if (iter > burnin)
  {
    n<-iter-burnin
    n1<-(n-1)/n
    alphasum<-n1*alphasum+alpha/n
    alpha2sum<-n1*alpha2sum+alpha^2/n
    alpha3sum<-n1*alpha3sum+alpha^3/n
    betasum<-n1*betasum+beta/n
    beta2sum<-n1*beta2sum+beta^2/n
    beta3sum<-n1*beta3sum+beta^3/n
    gammasum<-n1*gammasum+gamma/n
  }
...
return(list("beta"=betasum,"beta2"=beta2sum,"beta3"=beta3sum,"alpha"=alphasum,"alpha2"=alpha2sum,"alpha3"=alpha3sum,"gamma"=gammasum))
}
